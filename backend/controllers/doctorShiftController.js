...
const requireRoleDoctor = (req) => String(req.user?.role || "").toUpperCase() === "DOCTOR";
...
const getAssignedShiftType = async (user) => {
  const assignment = await doctorShiftModel.getAssignmentByUserId(user.id);
...
const buildStatusPayload = async (user) => {
  ...
  const latestSession = await doctorShiftModel.getLatestSessionByUserId(user.id);
...
const getShiftStatus = async (req, res) => {
  try {
    if (!requireRoleDoctor(req)) return res.status(403).json({ error: "Apenas médicos." });
...
const startShift = async (req, res) => {
  try {
    ...
    const latestSession = await doctorShiftModel.getLatestSessionByUserId(req.user.id);
    const now = new Date();
    ...
    if (!isExistingActive) {
      ...
      await doctorShiftModel.createShiftSession({
        userId: req.user.id,
        shiftType,
...
const extendShift = async (req, res) => {
  try {
    if (!requireRoleDoctor(req)) return res.status(403).json({ error: "Apenas médicos." });
    const session = await doctorShiftModel.getLatestSessionByUserId(req.user.id);
    ...
    await doctorShiftModel.extendLatestSession(session.id, minutes);
    return res.json({
...
const stopShift = async (req, res) => {
  try {
    ...
    await doctorShiftModel.stopLatestSession(session.id);
    return res.json({
...
const startBreak = async (_req, res) => {
  try {
    if (!requireRoleDoctor(req)) return res.status(403).json({ error: "Apenas médicos." });
    const session = await doctorShiftModel.getLatestSessionByUserId(req.user.id);
    if (!session?.clock_in_at || !isSessionActiveNow(session, new Date())) {
    ...
    await doctorShiftModel.startBreak(session.id);
    return res.json({
...
const resumeBreak = async (req, res) => {
  try {
    ...
    await doctorShiftModel.resumeBreak(session.id);
    return res.json({
