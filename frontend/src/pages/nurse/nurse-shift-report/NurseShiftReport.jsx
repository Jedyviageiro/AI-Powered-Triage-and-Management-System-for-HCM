...
export function NurseShiftReportView({
  me,
  queue = [],
  queueSummary = null,
  pastVisits = [],
  ...
  const avgWaitMinutes = useMemo(() => {
    const fromSummary = Number(
      queueSummary?.average_wait_minutes ?? queueSummary?.avg_wait_minutes
    );
  ...
  const destinations = useMemo(() => {
    const bucket = new Map();
    todayFinished.forEach((v) => {
      const key = String(v?.disposition_plan || "")
        .trim()
        .toUpperCase();
      if (!key) return;
      bucket.set(key, (bucket.get(key) || 0) + 1);
  ...
  }, [shiftHasStartedToday, shiftStatus?.is_on_shift]);

  const shiftWindowMeta = useMemo(() => {
    const displayWindow = shiftStatus?.shift_window || "-";
    const originalWindow = shiftStatus?.original_shift_window || null;
    let extensionMinutes = Number(shiftStatus?.extension_minutes || 0);
    if (!Number.isFinite(extensionMinutes) || extensionMinutes < 0) extensionMinutes = 0;

    if (extensionMinutes <= 0) {
      const scheduledEnd = shiftStatus?.scheduled_end
        ? new Date(shiftStatus.scheduled_end).getTime()
        : NaN;
      const extendedUntil = shiftStatus?.extended_until
        ? new Date(shiftStatus.extended_until).getTime()
        : NaN;
      if (
        Number.isFinite(scheduledEnd) &&
        Number.isFinite(extendedUntil) &&
        extendedUntil > scheduledEnd
      ) {
        extensionMinutes = Math.round((extendedUntil - scheduledEnd) / 60000);
      }
    }

    if (extensionMinutes <= 0 && shiftStatus?.shift_window && shiftStatus?.original_shift_window) {
      const currentEnd = parseHmToMinutes(String(shiftStatus.shift_window).split("-")[1]);
      const originalEnd = parseHmToMinutes(String(shiftStatus.original_shift_window).split("-")[1]);
      if (currentEnd != null && originalEnd != null) {
        const diff = currentEnd - originalEnd;
        if (diff > 0) extensionMinutes = diff;
      }
    }

    const isExtended = Boolean(shiftStatus?.is_extended) || extensionMinutes > 0;
    ...
    const extensionHoursRaw =
      extensionMinutes > 0 ? Math.round((extensionMinutes / 60) * 10) / 10 : 0;
    const extensionHoursValue = Number.isInteger(extensionHoursRaw)
      ? String(extensionHoursRaw)
      : String(extensionHoursRaw).replace(".", ",");
    const extensionHoursLabel =
      extensionMinutes > 0
        ? `+${extensionHoursValue} hora${extensionHoursRaw === 1 ? "" : "s"}`
        : "";
    return {
      displayWindow,
      originalWindow,
  ...
  return (
    <div>
      ...
      <div
        style={{
          background: "linear-gradient(145deg, #0c3a24 0%, #165034 55%, #1a6040 100%)",
          borderRadius: "18px",
          padding: "20px",
          color: "#ffffff",
          marginBottom: "14px",
      ...
      >
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr 0.9fr 0.9fr", gap: "10px" }}>
          ...
          >
            ...
            <div style={{ fontSize: "21px", lineHeight: 1.1, fontWeight: 800, marginTop: "6px" }}>
          ...
          >
            ...
            <div style={{ fontSize: "15px", lineHeight: 1.25, fontWeight: 700, marginTop: "6px" }}>
              {formatDate(shiftStatus?.clock_in_at)}
            </div>
          </div>
          <div
            style={{
          ...
          >
            ...
            <div style={{ fontSize: "17px", lineHeight: 1.2, fontWeight: 800, marginTop: "6px" }}>
              {shiftWindowMeta.displayWindow}
            </div>
            {shiftWindowMeta.isExtended && (
              <div
                style={{
                  marginTop: "8px",
                  display: "inline-flex",
              ...
              >
                ...
                />
                Turno estendido {shiftWindowMeta.extensionHoursLabel}
              </div>
            )}
            {shiftWindowMeta.isExtended && shiftWindowMeta.originalWindow && (
              <div style={{ marginTop: "5px", fontSize: "10px", opacity: 0.82 }}>
                Janela original: {shiftWindowMeta.originalWindow}
              </div>
            )}
          </div>
          <div
          ...
          >
            ...
            <div
              style={{
                ...
                borderRadius: "999px",
                padding: "5px 10px",
                background: shiftStateMeta.bg,
                border: `1px solid ${shiftStateMeta.border}`,
                color: shiftStateMeta.color,
                fontSize: "12px",
                fontWeight: 800,
              }}
            >
              <span
                style={{
                  ...
                  height: "8px",
                  borderRadius: "50%",
                  background: shiftStateMeta.dot,
                  boxShadow: `0 0 0 3px ${shiftStateMeta.bg}`,
                }}
              />
      ...
      >
        ...
        <div className="form-card" style={{ margin: 0, padding: "14px", position: "relative" }}>
          ...
          >
            {avgConsultMinutes != null ? `${avgConsultMinutes} min` : "-"}
          </div>
        </div>
      </div>
          ...
          <div style={{ height: "156px", display: "flex", alignItems: "flex-end", gap: "6px" }}>
            {hourlyFlow.data.map((h) => (
              <div key={h.label} style={{ flex: 1, minWidth: 0, textAlign: "center" }}>
                <div
                  style={{
                    height: `${(h.value / hourlyFlow.max) * 100}%`,
                    minHeight: h.value > 0 ? "8px" : "2px",
                    borderRadius: "6px 6px 0 0",
                    background: "linear-gradient(180deg,#2d6f4e,#165034)",
                  }}
                  title={`${h.label}: ${h.value}`}
                />
                <div style={{ fontSize: "10px", marginTop: "6px", color: "#6b7280" }}>
                  {h.label}
                </div>
              </div>
            ))}
          ...
          <div style={{ display: "grid", gap: "8px" }}>
            {topComplaints.length === 0 && (
              <div style={{ fontSize: "12px", color: "#9ca3af" }}>
                Sem dados de queixas para hoje.
              </div>
            )}
        ...
        <div className="form-card" style={{ margin: 0 }}>
          ...
          <div style={{ display: "grid", gap: "8px" }}>
            {destinations.length === 0 && (
              <div style={{ fontSize: "12px", color: "#9ca3af" }}>
                Sem registos de destino para hoje.
              </div>
            )}
...
export default function NurseShiftReport() {
  return <NursePage forcedView="shiftReport" />;
}
