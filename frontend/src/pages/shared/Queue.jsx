      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            {user?.role === "NURSE" && (
              <button onClick={() => navigate("/triage")} className="ui-btn ui-btn-ghost">
                Triagem
              </button>
            )}

            <button onClick={loadQueue} className="ui-btn ui-btn-ghost">
              Atualizar
            </button>
        ...
        <div className="bg-white rounded-2xl shadow overflow-hidden">
          ...
          {loading ? (
            ...
            queue.map((v) => (
              <div
                key={v.id}
                className={`grid grid-cols-7 gap-2 p-3 border-t text-sm ${rowBlink(v.is_overdue)}`}
              >
                ...
                <div>
                  ...
                  {v.is_overdue && (
                    <span className="ml-2 text-red-600 font-semibold">ATRASADO</span>
                  )}
                </div>
        ...
        <p className="text-xs text-slate-500 mt-3">Atualização automática a cada 5 segundos</p>
      </div>
    </div>
  );
}
